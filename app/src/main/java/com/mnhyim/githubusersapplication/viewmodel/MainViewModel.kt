    ...
    fun setRandomUsers() {
        ...
        client.get(url, object : AsyncHttpResponseHandler() {
            override fun onSuccess(
                ...
                val resultObj = JSONArray(result)

                for (i in 0 until resultObj.length()) {
                    val userTemp = resultObj.getJSONObject(i)
                    ...
                    Log.d(TAG, "setRandomUsers: onSuccess = $userItem")
            ...
            override fun onFailure(
                ...
                val errorMessage = when (statusCode) {
                    401 -> "Error $statusCode  : Bad Request"
                    403 -> "Error $statusCode : Forbidden"
                    404 -> "Error $statusCode : Not Found"
                    0 -> "Error $statusCode : No Internet Connection"
                    else -> "Error $statusCode : ${error.message}"
                }
                errorDesc.postValue(errorMessage)
                Log.d(TAG, "setRandomUsers: onFailure = " + error.message.toString())
            }
        })
    }
    ...
    fun setSearchUsers(username: String) {
        ...
        client.get(url, object : AsyncHttpResponseHandler() {
            override fun onSuccess(
                ...
                val resultArr = resultObj.getJSONArray("items")
                ...
                for (i in 0 until resultArr.length()) {
                    ...
                    val userItem = User(id, login, avatarURL, type, null, null, null, null)
                    Log.d(TAG, "setSearchUsers: onSuccess = $userItem")
            ...
            override fun onFailure(
                ...
                errorDesc.postValue(errorMessage)
                Log.d(TAG, "setSearchUsers: onFailure = " + error.message.toString())
            }
        })
    }
